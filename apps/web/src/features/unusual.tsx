...
export function UnusualActivity({ compact = false, selectedMonth, selectedCurrency }: {
    ...
    const c = data?.currencies.find(item => item.currency === (selectedCurrency ?? period.currency)) ?? (selectedCurrency ? undefined : data?.currencies[0]);
    if (compact)
        return <section className="unusual-summary panel"><span>{c ? `${c.total} ${c.state === "insufficient_history" ? "current-month signals; history developing" : c.total === 1 ? "unusual change" : "unusual changes"} · ${c.currency}` : error ? "Unusual activity unavailable" : "Your unusual activity"}</span><Link href="/app/unusual">Explore unusual activity ↗</Link></section>;
    ...
    {error ? <section className="empty"><p role="alert">{error}</p><button onClick={() => { setError(""); setRevision(r => r + 1); }}>Retry</button></section> : !data ? <p role="status">Comparing your imported history…</p> : !c ? <section className="overview-empty"><h2>Build your personal history</h2><p>Import statements to reveal supported changes over time.</p><Link href="/app/import">Import statement</Link></section> : <>
      ...
      <section className="unusual-lead"><strong>{c.total.toString().padStart(2, "0")}</strong><div><h2>{c.total === 0 ? "No supported signals to highlight" : c.total === 1 ? "Signal worth a closer look" : "Signals worth a closer look"}</h2><p>History-based reference: {c.prior_months.length} prior months · {c.historical_purchases} reference purchases</p></div></section>
      ...
      {c.state === "insufficient_history" && <p className="hint">Current-month purchase signals need at least six eligible purchases. They compare within this month, not against your historical normal.</p>}
      ...
      <ol className="unusual-list" key={`${data.month}-${c.currency}`}>{c.items.map((item, index) => <li className="unusual-card" key={item.identifier}>
        <div className="unusual-card-heading"><span className="rank-number">{String(index + 1).padStart(2, "0")}</span><div><h2>{item.subject}</h2><p><span>{item.basis === "current_month" ? "Current month" : "History-based"}</span> · <span>{item.day ? dateLabel(item.day) : data.month && monthLabel(data.month)}</span></p></div><span className={`unusual-severity ${item.severity.toLowerCase()}`}>{item.severity}</span></div>
