...
export function InsightFeed({ data, compact = false }: {
  ...
  <div className="insight-cards">{insights.slice(0, compact ? 2 : undefined).map((insight, i) => <article key={insight.code}><span className="insight-index">{String(i + 1).padStart(2, '0')}</span><div><span className="insight-kind">{insight.subject ?? insight.metric.replaceAll('_', ' ')}</span><h3>{insight.text}</h3>{!compact && <strong className="insight-value">{displayMoney(insight.delta, data.currency)}</strong>}<details><summary>See the calculation</summary><p>Selected month: {amount(insight.current, data.currency)}. Previous month: {amount(insight.previous, data.currency)}. Change: {displayMoney(insight.delta, data.currency)}.</p>{insight.total_delta !== null && <p>Total {insight.metric} change: {displayMoney(insight.total_delta, data.currency)}. {insight.contribution_percent !== null ? `${insight.contribution_percent}% contribution (individual change ÷ total change).` : 'No same-direction contribution is inferred.'} Category and merchant views overlap.</p>}</details>{!compact && <Link href={insight.subject ? '/app/behaviour' : '/app/trends'}>Explore {insight.subject ? 'behaviour' : 'trends'} ↗</Link>}</div></article>)}</div>
