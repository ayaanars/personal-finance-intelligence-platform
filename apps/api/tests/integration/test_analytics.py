...
def test_relationships_owned_finalized_window_currencies_and_replay(auth: Harness) -> None:
    endpoint = "/api/v1/analytics/relationships"
    assert auth.client.get(endpoint).status_code == 401
    headers = setup(auth)
    lines = [
        f"2026-{m:02d}-01,Salary,10000,AED\n2026-{m:02d}-05,Carrefour,-{m * 100},AED\n"
        for m in range(1, 10)
    ]
    lines += ["2026-09-05,Carrefour,-10,USD\n"]
    batch = upload(auth, headers, HEADER + "".join(lines).encode())
    assert auth.client.get(endpoint).json()["currencies"] == []
    final_headers = finalize_headers(headers)
    for _ in range(2):
        assert (
            auth.client.post(f"{BASE}/{batch}/finalize", json={}, headers=final_headers).status_code
            == 200
        )
    response = auth.client.get(endpoint)
    assert response.status_code == 200 and response.headers["cache-control"] == "no-store"
    data = response.json()
    aed, usd = data["currencies"]
    assert len(aed["observed_months"]) == 9
    item = next(i for i in aed["items"] if i["code"] == "spending_net")
    assert item["sample_count"] == 9 and item["points"][-1]["x"] == "900.0000"
    assert item["points"][-1]["y"] == "9100.0000"
    assert usd["state"] == "insufficient_history" and not usd["items"]
    assert data["window_start"] == "2025-09"
    with auth.engine.begin() as db:
        db.execute(text("DELETE FROM transaction_enrichments"))
    assert auth.client.get(endpoint).json() == data
    for params in ({"owner": str(uuid4())}, {"month": "0000-01"}):
        assert auth.client.get(endpoint, params=params).status_code == 422
    for month in ("0001-01", "9999-12"):
        assert auth.client.get(endpoint, params={"month": month}).status_code == 200
    assert (
        auth.client.get(endpoint, headers={"Origin": "https://foreign.invalid"}).status_code == 403
    )
    facts = auth.client.get("/api/v1/transactions").json()["items"]
    fact = next(f for f in facts if f["amount"] == "-900.0000")
    assert (
        auth.client.patch(
            f"/api/v1/transactions/{fact['id']}/category",
            json={"category": "Transfers"},
            headers={"X-CSRF-Token": auth.csrf(), "If-Match": '"0"'},
        ).status_code
        == 200
    )
    updated = auth.client.get(endpoint).json()
    assert updated["currencies"][1] == usd
    assert not any(i["code"] == "spending_net" for i in updated["currencies"][0]["items"])
    screening = next(
        s for s in updated["currencies"][0]["screening"] if s["code"] == "spending_net"
    )
    assert screening["state"] == "not_clear"
    totals = auth.client.get("/api/v1/analytics/intelligence").json()["currencies"][0]["totals"]
    assert totals["spending"] == "0.0000" and totals["net_cash_flow"] == "9100.0000"
    auth.register("relationships-other@example.com")
    auth.login("relationships-other@example.com")
    assert auth.client.get(endpoint).json()["currencies"] == []
    assert auth.client.get(endpoint, params={"month": "2026-09"}).json()["currencies"] == []
